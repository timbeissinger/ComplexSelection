...
### Start the loop over selection simulations. -- selection intensity 0.05
...
for(sim in 1:100){
	...
	#load genotypes and phenotypes
      	pheno <- read.table(paste(directory, "/Intensity0.05_data_",iter,".txt",sep=""),header=T,stringsAsFactors=F)
	map <- read.table(paste(directory, "/lm_mrk_",iter,".txt",sep=""),header=T,stringsAsFactors=F)

	geno <- read.table(paste(directory, "/Intensity0.05_mrk_",iter,".txt-head",sep=""),header=F,stringsAsFactors=F,skip=1,sep="", colClasses=c("character")) # read genos as characters #
	...
	#load allele frequencies
      	freqs0<-read.table(paste(directory, "/Intensity0.05_freq_mrk_",iter,".txt",sep=""),header=T,nrows=nrow(map),fill=T,stringsAsFactors=F)
	freqs20<-read.table(paste(directory, "/Intensity0.05_freq_mrk_",iter,".txt",sep=""),header=F,skip={nrow(map)+1},fill=T,stringsAsFactors=F) #
...
### Determine num_eff
    	...
    	ldTab <- read.table(paste(directory,"/Intensity0.05_ld_decay_",iter,".txt",sep=""),skip=43,nrows=15)
	...
	Pvals.05[as.numeric(iter)] <- test$p.val
}
```
...
hist(Pvals.05)
print(Pvals.05)
length(which(Pvals.05<=0.05))/length(Pvals.05)



save.image("intensity05.RData")
```