...
### Start the loop over selection simulations. -- selection intensity 0.5
...
for(sim in 1:100){
	...
	#load genotypes and phenotypes
      	pheno <- read.table(paste(directory, "/Intensity0.5_data_",iter,".txt",sep=""),header=T,stringsAsFactors=F)
	map <- read.table(paste(directory, "/lm_mrk_",iter,".txt",sep=""),header=T,stringsAsFactors=F)

	geno <- read.table(paste(directory, "/Intensity0.5_mrk_",iter,".txt-head",sep=""),header=F,stringsAsFactors=F,skip=1,sep="", colClasses=c("character")) # read genos as characters #
	...
	#load allele frequencies
      	freqs0<-read.table(paste(directory, "/Intensity0.5_freq_mrk_",iter,".txt",sep=""),header=T,nrows=nrow(map),fill=T,stringsAsFactors=F)
	freqs20<-read.table(paste(directory, "/Intensity0.5_freq_mrk_",iter,".txt",sep=""),header=F,skip={nrow(map)+1},fill=T,stringsAsFactors=F) #
...
### Determine num_eff
    	...
    	ldTab <- read.table(paste(directory,"/Intensity0.5_ld_decay_",iter,".txt",sep=""),skip=43,nrows=15)
	...
	Pvals.5[as.numeric(iter)] <- test$p.val
}
```
...
hist(Pvals.5)
print(Pvals.5)
length(which(Pvals.5<=0.05))/length(Pvals.5)


save.image("intensity5.RData")
```