---
title: "Analysis of the effect that selection intensity has on Ghat with 1000-QTL simulated QTL"
...
### Start the loop over selection simulations. -- selection intensity 0.01
...
for(sim in 1:100){
	...
	directory="/home/beissingert/data/PolySelect_bigsims/Selection_Intensity/r_intensity"

	#load genotypes and phenotypes
      	pheno <- read.table(paste(directory, "/Intensity0.01_data_",iter,".txt",sep=""),header=T,stringsAsFactors=F)
	map <- read.table(paste(directory, "/lm_mrk_",iter,".txt",sep=""),header=T,stringsAsFactors=F)

	geno <- read.table(paste(directory, "/Intensity0.01_mrk_",iter,".txt-head",sep=""),header=F,stringsAsFactors=F,skip=1,sep="", colClasses=c("character")) # read genos as characters #
	ids <- as.numeric(substr(geno[,1],1,7)) #
	markers <- substr(geno[,1],8,200000) #
	geno <- data.frame(V1=ids,V2=markers,stringsAsFactors=F) #

	qtlMap <- read.table(paste(directory, "/lm_qtl_",iter,".txt",sep=""),header=T,stringsAsFactors=F)

	#load allele frequencies
      	freqs0<-read.table(paste(directory, "/Intensity0.01_freq_mrk_",iter,".txt",sep=""),header=T,nrows=nrow(map),fill=T,stringsAsFactors=F)
	freqs20<-read.table(paste(directory, "/Intensity0.01_freq_mrk_",iter,".txt",sep=""),header=F,skip={nrow(map)+1},fill=T,stringsAsFactors=F) #
...
### Determine num_eff
    	...
    	ldTab <- read.table(paste(directory,"/Intensity0.01_ld_decay_",iter,".txt",sep=""),skip=43,nrows=15)
	...
	Pvals.01[as.numeric(iter)] <- test$p.val
}
```
...
hist(Pvals.01)
print(Pvals.01)
length(which(Pvals.01<=0.05))/length(Pvals.01)


save.image("intensity01.RData")
```